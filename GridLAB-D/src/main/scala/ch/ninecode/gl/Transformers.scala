...
class Transformers (session: SparkSession, storage_level: StorageLevel = StorageLevel.fromString ("MEMORY_AND_DISK_SER")) extends CIMRDD with Serializable
{
    ...
    ): RDD[TData] =
    {
        ...
        def station_fn (x: (PowerTransformer, Element)): (String, PowerTransformer) =
        {
            x match
            {
                case (t: PowerTransformer, station: Substation) => (station.id, t)
                case (t: PowerTransformer, bay: Bay) => (bay.Substation, t)
                case (t: PowerTransformer, level: VoltageLevel) => (level.Substation, t)
        ...
        val tpairs = if (null != get[EquipmentContainer])
            substation_transformers.keyBy (_.ConductingEquipment.Equipment.EquipmentContainer).join (elements.keyBy (_.id)).values.map (station_fn)
        else
    ...
    ): RDD[TransformerData] =
    {
        ...
        // get the transformers of interest and join to end information
        val transformers = getOrElse[PowerTransformer].filter (transformer_filter)
            .keyBy (_.id).join (ends_terminals_voltages.keyBy (_._1.PowerTransformer).groupByKey)
