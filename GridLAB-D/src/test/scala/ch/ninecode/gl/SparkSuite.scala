...
class SparkSuite extends FunSuite
{
    ...
    class Unzip
    {
        ...
        def unzip (file: String, directory: String): Unit =
        {
            ...
            while (null != entry) // iterates over entries in the zip file
            {
                val path = directory + entry.getName
                if (!entry.isDirectory)
                    extractFile (zip, path) // if the entry is a file, extracts it
                else
                    new File (path).mkdir // if the entry is a directory, make the directory
                zip.closeEntry ()
