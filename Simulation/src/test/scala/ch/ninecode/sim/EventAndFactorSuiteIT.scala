...
class EventAndFactorSuiteIT
{
    ...
    @Test def events ()
    {
        ...
        val access = new SimulationCassandraAccess(_Spark, org.apache.spark.storage.StorageLevel.MEMORY_AND_DISK_SER, ID, INPUT_KEYSPACE, OUTPUT_KEYSPACE, verbose = true)
        check.run(access)
    }
