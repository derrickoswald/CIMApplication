...
extends CIMRDD
{
    ...
    def read_csv (csv: String): RDD[EquivalentInjection] =
    {
        ...
        def toEquivalentInjection (voltages: Map[Double, String])(row: Row): EquivalentInjection =
        {
            ...
            val obj = IdentifiedObject (sup = element, aliasName = sap, description = description, mRID = mRID, name = s"$name equivalent injection")
    ...
    def toTerminalsAndLocations (pair: (EquivalentInjection, TransformerDetails)): List[Element] =
    {
        ...
        if (eq_inj.EquivalentEquipment.ConductingEquipment.BaseVoltage == details.voltage)
        {
            ...
            val obj = IdentifiedObject (sup = BasicElement (mRID = mRID), aliasName = eq_inj.id, description = old_obj.description, mRID = mRID, name = old_obj.name, DiagramObjects = old_obj.DiagramObjects, Names = old_obj.Names)
