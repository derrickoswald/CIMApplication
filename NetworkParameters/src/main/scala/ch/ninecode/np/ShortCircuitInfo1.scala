...
object ShortCircuitInfo1 extends CIMInitializer[NetworkParametersOptions] with Main
{
    def run (options: NetworkParametersOptions): Unit =
    {
        if (options.verbose)
        {
            ...
            LogManager.getLogger (classOf[ShortCircuitInfo1]).setLevel (org.apache.log4j.Level.INFO)
        }
        if (options.main_options.valid)
        {
            if (options.cim_options.files.nonEmpty)
            {
                if ("" != options.available_power_csv)
                {
                    ...
                    {
                        ShortCircuitInfo1 (session, options).run ()
                    }
                }
                else
    ...
    def main (args: Array[String])
    {
        ...
        else
        {
            ...
            val default = NetworkParametersOptions (
                main_options = MainOptions (s"Customer1_NetworkParameters", application_version),
                spark_options = SparkOptions (jars = jars, kryo = kryo),
                cim_options = temp.cim_options.copy (options = temp.cim_options.toMap),
                available_power_csv = "KS_Leistungen.csv"
            )

            // parse the command line arguments
            new Customer1OptionsParser (default).parse (args, default) match
            {
                case Some (options) =>
