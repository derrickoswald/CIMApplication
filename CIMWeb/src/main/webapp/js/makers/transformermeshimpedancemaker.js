...
`;
            ...
            submit_parameters ()
            {
                ...
                const transformer_name = document.getElementById ("transformer_name").value;

                // add power transformer ends
                let endinfos = null;
                let tankinfo = this._cimmap.fetch ("TransformerTankInfo", info => info.PowerTransformerInfo === transformer_name);
            ...
            {
                let ret = 0.5; // default from DACHCZ Appendix E
                if (ratedS < 500000)
                {
                    const x0 = Math.log (50000);
                    const y0 = 1.0;
                    const x1 = Math.log (500000);
                    const y1 = 3.4;
                    ret = ((Math.log (ratedS) - x0) * (y1 - y0) / (x1 - x0)) + y0;
                }
                else
                {
                    const x0 = Math.log (500000);
                    const y0 = 4.0;
                    const x1 = Math.log (3000000);
                    const y1 = 8.0;
                    ret = ((Math.log (ratedS) - x0) * (y1 - y0) / (x1 - x0)) + y0;
                }
                return (ret);
            ...
            {
                const from = parameters.ToTransformerEndInfos.find (x => x.id === mesh.FromTransformerEndInfo); // winding under consideration
                const s = from.ratedS;
                const v = from.ratedU;
                const z_p_u = parameters.impedance_voltage / 100.0; // from nameplate in percent
                const x_r = (0 === parameters.x_r_ratio) ? this.estimate_x_r_ratio (s) : parameters.x_r_ratio;
                // compute per unit values from per unit Z and the ratio
                const r_pu = Math.sqrt (z_p_u * z_p_u / (1.0 + (x_r * x_r)));
                const x_pu = x_r * r_pu;
                // convert to absolute units using the voltage and power
                const base_ohms = v * v / s;
