...
class CIMReaderOptionsParser[T <: Mainable with Sparkable with CIMAble] (default: T) extends SparkOptionsParser[T](default)
{
    ...
    opt[Map[String,String]]("cim_options")
        .valueName ("<map>")
        .action ((x, c) => { c.cim_options = CIMReaderOptions (x, Some (c.cim_options)); c })
        .text (s"CIM options [${default.cim_options.options.map (x => s"${x._1}$EQUAL${x._2}").mkString (COMMA)}]")

    val children = List (
        opt[Unit]("identify_islands")
            .action ((_, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (identify_islands = true)); c })
        ...
        opt[State]("retain_switch")
            .valueName ("<state>")
            .action ((s, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (force_retain_switches = s)); c })
        ...
        opt[State]("retain_fuse")
            .valueName ("<state>")
            .action ((s, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (force_retain_fuses = s)); c })
        ...
        opt[State]("switch_island")
            .valueName ("<state>")
            .action ((s, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (force_switch_separate_islands = s)); c })
        ...
        opt[State]("fuse_island")
            .valueName ("<state>")
            .action ((s, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (force_fuse_separate_islands = s)); c })
        ...
        opt[Unit]("default_open")
            .action ((s, c) => { c.cim_options = c.cim_options.copy (topology_options = c.cim_options.topology_options.copy (default_switch_open_state = true)); c })
    ...
    opt[Unit]("topology")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (topology = true); c })
    ...
    opt[Unit]("about")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (about = true); c })
        .text (s"do about processing [${default.cim_options.about}]")

    opt[Unit]("normalize")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (normalize = true); c })
        .text (s"do normalization processing [${default.cim_options.normalize}]")

    opt[Unit]("dedup")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (dedup = true); c })
        .text (s"do deduplication processing [${default.cim_options.dedup}]")

    opt[Unit]("edges")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (edges = true); c })
        .text (s"do edge processing [${default.cim_options.edges}]")

    opt[Unit]("join")
        .action ((_, c) => { c.cim_options = c.cim_options.copy (join = true); c })
    ...
    opt[Long]("splitsize")
        .action ((l, c) => { c.cim_options = c.cim_options.copy (splitsize = l); c })
        .text (s"file read split size [${default.cim_options.splitsize}]")

    opt[String]("cache")
        .valueName ("<dir>")
        .action ((s, c) => { c.cim_options = c.cim_options.copy (cache = s); c })
    ...
    arg[String]("<CIM> <CIM> ...")
        .optional ()
        .unbounded ()
        .action ((x, c) => { c.cim_options = c.cim_options.copy (files = c.cim_options.files :+ x); c })
